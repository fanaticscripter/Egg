      ...
      <template v-if="!artifact.notDroppableInPractice && obtainableMissions.length > 0">
        ...
        <div class="text-xs text-red-600 space-y-1">
          <p>
            ...
            Absolute values are of limited value at the moment, but most relative comparisons are still valid.
          </p>
          <p v-if="showDropRateWarning(artifact)">
            This item did not drop prior to 2021-05-20 thanks to a server bug, which was fixed
            "unintentionally" that day. Therefore, the following drop rates may differ from actual
            rates by a lot.
          </p>
        </div>
        <ul class="grid grid-cols-1 gap-x-4 gap-y-1 sm:grid-cols-2 xl:grid-cols-3">
